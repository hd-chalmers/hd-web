<v-app-bar app elevate-on-scroll>
    ...
    <v-toolbar-items class="hidden-lg-and-up">
        <v-btn icon x-large href="{{route('committee.index')}}">
            <v-icon title="Sittande">mdi-account-group</v-icon>
        </v-btn>
        <v-btn icon x-large href="{{route('event.index')}}">
            <v-icon title="Evenemang">mdi-calendar-star</v-icon>
        </v-btn>
        <v-btn icon x-large href="{{route('products.index')}}">
            <v-icon title="Prislista">mdi-cash</v-icon>
        </v-btn>
        @if (\Illuminate\Support\Facades\Auth::check())
            <v-btn icon x-large href="{{route('account.index')}}">
                <v-icon title="Strecklista">mdi-format-list-checkbox</v-icon>
            </v-btn>
        @endif
        <v-btn icon x-large href="{{route('games.index')}}">
            <v-icon title="Våra Spel">mdi-gamepad-variant</v-icon>
        </v-btn>
        <v-btn icon x-large href="{{route('contact')}}">
            <v-icon title="Kontakta Oss">mdi-phone</v-icon>
        </v-btn>
        <v-btn icon x-large href="https://www.facebook.com/HsektionenChalmers/">
            <span style="color: #E0218A">H</span>
        </v-btn>
        <v-btn color="blue" icon x-large href="https://www.facebook.com/HDChalmers/">
            <v-icon>mdi-facebook</v-icon>
        </v-btn>
        <!--<v-btn icon href="https://www.facebook.com/HDChalmers/"><v-icon>mdi-instagram</v-icon></v-btn>-->
    </v-toolbar-items>

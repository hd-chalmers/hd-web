        ...
        <v-card-title>
            <v-row>
                <v-col cols="12" sm="4" md="3">
                    ...
                    <v-btn v-if="all_years[0] === undefined || (all_years[0] && new Date(all_years[0].year).getFullYear() !== (new Date().getFullYear()))"
...
import {ActiveYearData, LoehkMemberType} from "@/assets/interfaces";
...
@Component<LoehkActiveYear>({
  ...
  watch: {
    'form.front_image': function ($value) {
      ...
      //this.state.then(obj => {
        fetch(process.env.NUXT_ENV_API_URL + `/loehk/active_year/${this.active_year.id}`, {
    ...
    'form.background_image': function ($value) {
      if (this.loading) {
        return
      }
    ...
    'form.group_photo': function ($value) {
      if (this.loading) {
        return
      }
    ...
    'form.christmas_image': function ($value) {
      if (this.loading) {
        return
      }
...
export default class LoehkActiveYear extends Vue{
  constructor() {
    super()
    this.getActiveYear()
  }
  ...
  load = {
    group_photo: false as boolean | string,
    front_image: false as boolean | string,
    christmas_image: false as boolean | string,
    background_image: false as boolean | string,
    description: false as boolean | string,
    addYear: false as boolean | string,
    addMember: false as boolean | string,
    committee_member: [] as (boolean | string)[],
  }
  all_years: ActiveYearData[] = []
  committee_member: LoehkMemberType = {
    name: '',
    role: '',
    image: '',
    description: '',
  ...
  _updateCommitteeDebounce: NodeJS.Timeout | undefined
  _SaveDescriptionDebounce: NodeJS.Timeout | undefined

  getMessage(
    type: "description" | "group_photo" | "front_image" | "christmas_image" | "background_image" | "committee_member",
    id: any
  ): string {
  ...
  addNewYear(): void {
    ...
    this.errors.addYear = ''
    //this.state.then(obj => {
      fetch(process.env.NUXT_ENV_API_URL + `/loehk/active_year`, {
  ...
  updateCommitteeMemberDebounced(id: number, fieldname: string, value: string | Blob, index: number | string): void {
  ...
  updateCommitteeMember(id: number, fieldname: string, value: string | Blob, index: number | string): void {
    ...
    data.append('active_year_id', this.active_year.id.toString());
    ...
    //this.state.then(obj => {
      fetch(process.env.NUXT_ENV_API_URL + `/loehk/active_year/committee_members${id ? '/' + id : ''}`, {
  ...
  deleteCommitteeMember(member: LoehkMemberType, index: number): void {
    ...
    //this.state.then(obj => {
      fetch(process.env.NUXT_ENV_API_URL + `/loehk/active_year/committee_members/${member.id}`, {

  ...
  <v-card>
        ...
        <v-card-text>
            <v-data-table :items="accounts"
                          :headers="$vuetify.breakpoint.xs ? [headers[1]] :headers"
                          item-key="id"
                          :loading="loading"
...
import {InvestAccData} from "@/assets/interfaces";
...
headers = [
  ...
  deleteAccount(item: InvestAccData): void {
    ...
    //this.state.then(obj => {
      fetch(process.env.NUXT_ENV_API_URL + `/loehk/investments/` + item.id, {
  ...
  createAccount(item: { uid: string, name: string, password: string | null }): void {
    ...
    //this.state.then(obj => {
      fetch(process.env.NUXT_ENV_API_URL + `/loehk/investments`, {

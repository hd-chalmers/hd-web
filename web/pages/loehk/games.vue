...
export default class Games extends Vue{
  ...
  getData(): void{
    ...
    fetch(process.env.NUXT_ENV_API_URL + '/loehk/games', {
      headers:{
...
/*
  ...
  */
  ...
  deleteGame(item: LoehkGameData): void{
    ...
    fetch(process.env.NUXT_ENV_API_URL + "/loehk/games/" + item.id, {
      method: "DELETE",
      headers:{
        Authorization: SessionStore.getSessionId() ?? ""
      }
  ...
  searchBoardgameAtlas(item: LoehkGameData): void{
    ...
    fetch(`https://api.boardgameatlas.com/api/search?client_id=${process.env.NUXT_ENV_BOARDGAME_ATLAS_KEY}&limit=6&name=` + item.name)
      .then(res => res.json()).then((res: {games: BoardgameAtlasGame[], count: number}) => {
        ...
        if(this.mechanicsList[0]){
          ...
          fetch('https://api.boardgameatlas.com/api/game/mechanics?client_id=' + process.env.NUXT_ENV_BOARDGAME_ATLAS_KEY)
